...
namespace ZhenxiRunTime::JniTrace {
    static HookFunType hook_func_instance = nullptr;
...
# define IS_MATCH \
    ...
    static void write(const std::string &msg) {
        ...
        if (strstr(msg.c_str(), "invoke method")) {
            GetStackTraceInfoByBuiltin();
        }
    }
    ...
    JNI_HOOK_DEF(jobject, CallObjectMethodV, JNIEnv *env, jobject obj, jmethodID jmethodId,
        ...
        DL_INFO
        IS_MATCH
                LOG(INFO) << "invoke method : CallObjectMethodV";
    ...
    JNI_HOOK_DEF(jboolean, CallBooleanMethod, JNIEnv *env, jobject obj, jmethodID jmethodId,
                 va_list args)

        DL_INFO
        IS_MATCH
                GET_JOBJECT_INFO(env, obj, "CallBooleanMethod")
                GET_METHOD_INFO_ARGS(env, obj, jmethodId, args, false)
                jboolean ret = orig_CallBooleanMethod(env, obj, jmethodId, args);
        ...
        return orig_CallBooleanMethod(env, obj, jmethodId, args);
    }
    ...
    void getJObjectInfo(JNIEnv *env, jobject obj, const string &methodname) {
        ...
        write(string("").append(temptag).append("\n"), true);
...
void Jnitrace::startjnitrace(JNIEnv *env, const std::list<string> &filter_list, char *path, HookFunType hook_func) {
    filterSoList = filter_list;
    hook_func_instance = hook_func;
    ...
    HOOK_JNITRACE(env, CallBooleanMethod)
